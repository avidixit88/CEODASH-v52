...
def _sponsor_evidence_read(summary: SponsorEvidenceSummary | None) -> str | None:
    """Translate sponsor news evidence into a concise adaptive executive fragment."""
    if summary is None:
        return None
    checked = ", ".join(summary.sponsors_checked[:10])
    if summary.result_items:
        top = summary.result_items[:3]
        pieces = [f"{item.sponsor}: {item.title}" for item in top]
        return (
            "Filtered sponsor evidence check found recent result/safety language with sponsor and topic overlap: "
            + "; ".join(pieces)
            + ". Treat media/news items as evidence leads until reconciled against the sponsor release, conference abstract, or trial population."
        )
    if summary.timing_items:
        top = summary.timing_items[:3]
        pieces = [f"{item.sponsor}: {item.title}" for item in top]
        return (
            "Filtered sponsor evidence check found possible data-timing or conference signals with monitored-lane overlap: "
            + "; ".join(pieces)
            + ". This shifts the watch item from protocol structure toward when external data may become available."
        )
    if summary.sponsors_checked:
        return (
            f"Sponsor evidence check did not surface recent ORR/PFS/DOR/safety disclosure language from dynamically discovered sponsors checked ({checked}). "
            "Current CDH6 read therefore remains protocol-and-battlefield intelligence, not sponsor-reported efficacy benchmarking."
        )
    return "Sponsor evidence check did not have public-company/news handles for the surfaced sponsors in this run, but the dynamic sponsor registry still preserves the sponsor names and evidence-search links for follow-up."
...
def _sponsor_evidence_trend(summary: SponsorEvidenceSummary | None) -> str | None:
    ...
    if summary.result_items:
        ...
        return f"Reported-data watch: dynamic sponsor evidence surfaced outcome/safety language from {sponsors}; these items should be reconciled against the related trial population, monotherapy/combination setting, and endpoint definitions."
    if summary.timing_items:
        sponsors = ", ".join(dict.fromkeys(item.sponsor for item in summary.timing_items[:4]))
        return f"Upcoming-data watch: dynamic sponsor evidence surfaced conference or readout timing language from {sponsors}; future runs should watch whether this becomes actual ORR/PFS/DOR/safety evidence."
    return None
